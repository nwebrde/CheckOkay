...
>(({ className, children, ...props }, ref) => {
    ...
    return (
        <DialogPrimitive.Overlay
            ...
            className={cn('transition-all delay-1000 ease-in duration-1000 z-50 flex bg-black/80 justify-center items-center p-2',
                open ? 'opacity-1' : 'opacity-0',
                className)}
            {...props}
            ref={ref}
        >
            <Animated.View entering={FadeIn.duration(2000)} exiting={FadeOut.duration(2000)}>
                <>{children}</>
            </Animated.View>
        </DialogPrimitive.Overlay>
    );
